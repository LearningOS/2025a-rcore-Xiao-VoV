...
use crate::task::{
    ...
    suspend_current_and_run_next,unmap_area,get_systrace
};
...
pub fn sys_get_time(ts: *mut TimeVal, _tz: usize) -> isize {
    ...
    // 简单地按字节逐个拷贝，效率较低
    let v: alloc::vec::Vec<&'static mut [u8]> =
        translated_byte_buffer(token, data_ptr, size_of::<TimeVal>());
...
pub fn sys_mmap(start: usize, len: usize, port: usize) -> isize {
    ...
    map_new_area(start, len, permission);
    0
}
